...
def ask1():
    ...
    for i in frame_list:
        ...
        for k in frame_list1:
            frameForth_dir = frame_dir + '/' + i + '/' + k
            if not os.path.exists(frameForth_dir + '/gap.txt'):
                continue
            le = len(k)
            bv_name = ''
            fl = 0
            ...
            if not BV_list.get(bv_name):
                continue
            with open(frameForth_dir + '/gap.txt', 'rt', encoding='utf-8') as f:
                for line in f:
                    tmp1 = line.split()
                    tp0 = int(tmp1[0])
                    tp1 = int(tmp1[1])
    ...
    with open('./ForthUse.txt', 'w', encoding='utf-8') as f:
