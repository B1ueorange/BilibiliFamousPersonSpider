...
def main():
    ...
    #     sz.append(0)
    l = ''
    list1 = os.listdir('./VideoClip')
    for i in list1:
        list2 = os.listdir('./VideoClip/' + i)
        ans2 = 0
        for j in list2:
            list3 = os.listdir('./VideoClip/' + i + '/' + j)
            ans += len(list3)
            ans2 += len(list3)
        l += i + ' ' + str(ans2) + ' ' + str(float(ans2 / 900)) + '\n'
    print(ans)
    with open('./FinalCutNumber.txt', 'w', encoding='utf-8') as f:
        f.writelines(l)
    # try:
