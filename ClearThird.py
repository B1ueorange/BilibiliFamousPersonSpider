...
def calc(num):
    ...
    for i in second_frame_list:
        ...
        if not os.path.exists('./frameThird/' + i):
            os.makedirs('./frameThird/' + i)
        for j in second_person_frame_list:
            ...
            if(os.path.exists('./frameThird/' + i + '/' + j + '/gap.txt')):
                continue
            if not os.path.exists('./frameThird/' + i + '/' + j):
                os.makedirs('./frameThird/' + i + '/' + j)
            le = len(frame_list)
            flag1 = []
            for k in range(0, le):
                flag1.append(0)
            ...
            with open('./frameSecond/' + i + '/' + j + '/gap.txt', 'rt', encoding = 'utf-8') as f:
                for line in f:
                    ...
                    for k in range(ll, rr + 1):
                        ...
                        result = face_recognition.compare_faces([unknown_face_encoding], personal_face_encoding, 0.39)
                        if(result[0] == True):
                            flag1[k] = 1
                        if(flag1[k] == 1):
                            if(fl1 == 0):
                    ...
                    if (fl1 == 1):
                        s += str(l) + ' ' + str(rr) + '\n'
                        fl1 = 0
            with open('./frameThird/' + i + '/' + j + '/gap.txt', 'w', encoding = 'utf-8') as f:
                f.writelines(s)
